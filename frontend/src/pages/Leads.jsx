...
const CITIES_RAW = [
  ...
  'Concord, CA','Santa Clara, CA','Victorville, CA',
  ...
  'Pompano Beach, FL','West Palm Beach, FL','Lakeland, FL','Fort Myers, FL',
  'New York City, NY','Buffalo, NY','Rochester, NY','Yonkers, NY','Syracuse, NY',
  'Albany, NY','Brooklyn, NY','Queens, NY','Manhattan, NY','Staten Island, NY','The Bronx, NY',
...
const NICHES_RAW = [
  ...
  'Ice Cream Shops','Juice Bars','Food Catering',
  ...
  'Fence Installation','Deck Building','Foundation Repair','Appliance Repair',
  'Auto Repair','Auto Body Shops','Car Washes','Oil Change Shops','Tire Shops',
  'Auto Detailing','Towing Services','Windshield Repair','Transmission Repair',
  'Hair Salons','Barbershops','Nail Salons','Spas','Massage Therapy',
  'Eyebrow Threading','Waxing Salons','Tattoo Shops','Lash Studios',
  'Gyms','Personal Trainers','Yoga Studios','Pilates Studios','CrossFit Gyms',
  'Martial Arts','Boxing Gyms','Dance Studios',
  'Dentists','Orthodontists','Chiropractors','Physical Therapy','Optometrists',
  'Veterinarians','Pet Grooming','Animal Hospitals',
  'Accountants','Tax Services','Lawyers','Real Estate Agents','Insurance Agents',
  'Tutoring Centers','Driving Schools','Music Lessons','Daycare Centers',
  'Florists','Photographers','Videographers','Event Planners','Wedding Planners',
  'Locksmiths','Solar Installation','Concrete Contractors','Asphalt Paving',
  'Printing Services','Sign Shops','Dry Cleaners','Shoe Repair',
...
const makeSelectStyles = (minH = '42px') => ({
  control: (b, s) => ({
    ...
    minHeight: minH, cursor: 'text', transition: 'border-color 0.2s, box-shadow 0.2s',
    '&:hover': { borderColor: 'rgba(139,92,246,0.4)' },
  }),
  menuPortal: b => ({ ...b, zIndex: 9999 }),
  menu: b => ({
    ...b, fontFamily: siteFont, background: '#111118',
    border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px',
    boxShadow: '0 24px 64px rgba(0,0,0,0.85)', zIndex: 9999, overflow: 'hidden',
  }),
  menuList: b => ({ ...b, fontFamily: siteFont }),
  option: (b, s) => ({
    ...b, fontFamily: siteFont,
    background: s.isSelected ? 'rgba(139,92,246,0.18)' : s.isFocused ? 'rgba(139,92,246,0.08)' : 'transparent',
    color: s.isSelected ? '#c4b5fd' : s.isFocused ? '#d4d4d8' : '#c4c4cc',
    borderRadius: '7px', fontSize: '0.875rem', padding: '9px 12px', cursor: 'pointer',
  }),
  ...
  dropdownIndicator: b => ({ ...b, color: '#b8c2d4', paddingRight: '12px', '&:hover': { color: '#8b5cf6' } }),
  clearIndicator: b => ({ ...b, color: '#b8c2d4', padding: '0 6px', cursor: 'pointer', '&:hover': { color: '#f87171' } }),
...
// ─── Scrape Animation (local display, reads from context) ─────────────────────

function ScrapeAnimation({ onDismiss }) {
  const { job, jobId, scrapeQuery, scrapeLocation, stopScrape } = useScrape()
      ...
      <div style={{ padding: '20px 22px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: '#b8c2d4', fontFamily: "'JetBrains Mono', monospace", marginTop: 1 }}>{scrapeQuery?.value} · {scrapeLocation?.value}</div>
                </div>
              </div>
        ...
        {isFinished && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button onClick={onDismiss} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#b8c2d4', padding: 6, display: 'flex' }}>
              <X size={14} />
            </button>
          </div>
        )}
    ...
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      ...
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, zIndex: 50, background: '#111118', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: 4, minWidth: 130, boxShadow: '0 20px 50px rgba(0,0,0,0.6)' }}>
          {STAGES.map(stage => {
            const c = STAGE_STYLE[stage]; const active = stage === (lead.pipeline_stage || 'New Lead')
            return (
    ...
    <div style={{ animation: 'fadeUp 0.4s cubic-bezier(0.16,1,0.3,1) both' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => navigate('/batches')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: 8, padding: '7px 14px', color: '#a78bfa', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: "'Outfit', sans-serif" }}>
            View in Batches <ArrowRight size={12} />
          </button>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#b8c2d4', padding: 6, display: 'flex' }}>
            <X size={14} />
          </button>
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.015)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, overflow: 'hidden', position: 'relative', minWidth: 580 }}>
        ...
        {leads.map(lead => {
          ...
          return (
            <div key={lead.id}
              style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1.2fr 1.2fr 1fr', alignItems: 'center', gap: 16, padding: '12px 18px', borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.15s' }}
...
export default function Leads() {
  ...
  const navigate = useNavigate()
  const {
    job, scrapeQuery, scrapeLocation, isScraping,
    completedBatchId, showResults,
    startScrape, dismissJob, setShowResults,
    registerRefetch,
  } = useScrape()
  ...
  // Local form state (only needed while on this page)
  const [formQuery, setFormQuery]       = useState(scrapeQuery)
  const [formLocation, setFormLocation] = useState(scrapeLocation)
  ...
  // Register refetch so context can trigger it when scrape finishes
  useEffect(() => {
    registerRefetch(fetchBatches)
  ...
  // Dup detection
  useEffect(() => {
    if (!formQuery || !formLocation) { setDupWarning(null); return }
    const match = batches.find(
      b => b.query.toLowerCase() === formQuery.value.toLowerCase() &&
           b.location.toLowerCase() === formLocation.value.toLowerCase()
    )
    setDupWarning(match || null)
  }, [formQuery, formLocation, batches])

  const handleScrape = () => {
    if (!formQuery || !formLocation) return
    startScrape(formQuery, formLocation, noWebOnly)
  }
      ...
      <div className="page-content leads-layout" style={{ position:'relative',zIndex:1,maxWidth:1280,margin:'0 auto',padding:'48px 48px 80px',display:'grid',gridTemplateColumns:'1fr 300px',gap:32,alignItems:'start' }}>
        ...
        <div>
          ...
          <div className="f2" style={{ marginBottom: 28 }}>
            {/* Active animation — only when scraping/finished and job exists */}
            {job && (
              <div style={{ marginBottom: showResults ? 24 : 0 }}>
                <ScrapeAnimation onDismiss={dismissJob} />
              </div>
            )}
                ...
                <div style={{ padding:'28px 28px 24px' }}>
                  ...
                  <div style={{ display:'flex',flexDirection:'column',gap:14 }}>
                    ...
                    <div className="scan-action-row" style={{ display:'flex',alignItems:'center',justifyContent:'space-between' }}>
                      ...
                      <button className="scan-btn" onClick={handleScrape} disabled={!formQuery || !formLocation}>
            ...
            {showResults && completedBatchId && !isScraping && (
              <BatchResults
                ...
                onClose={() => { setShowResults(false); dismissJob() }}
              />
            )}
          </div>
